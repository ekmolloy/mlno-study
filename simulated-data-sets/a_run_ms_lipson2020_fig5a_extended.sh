...
SU=20   # 5 - Ulchi
SM2=20  # 6 - Mixe 2
NSAMP=$(echo "$SB + $SF + $SH + $SM1 + $SU + $SM2" | bc)
...
# Scale the effective population size on each branch
# so each branch is the right length in terms of CUs
...
X_M1=$(echo "($ET_M1 / (2 * $ECU_M1)) / $PSIZE" | bc -l)
X_M2=$(echo "($ET_M2 / (2 * $ECU_M2)) / $PSIZE" | bc -l)
...
if [ ! -e ms_output_lipson2020_fig5a_extended.txt.gz ]; then
    echo "3579 27011 59243" > seedms

    ms $NSAMP $NREPL -T \
        -t $THETA \
        -r $RHO $NSITE \
        -I $NPOP $SB $SF $SH $SM1 $SU $SM2 \
        -en 0 1 $X_B \
        -en 0 2 $X_F \
        -en 0 3 $X_H \
        -en 0 4 $X_M1 \
        ...
        -en $T_FHMU 2 $X_FHMU \
        -ej $T_BFHMU 2 1 \
        > ms_output_lipson2020_fig5a_extended.txt

    gzip ms_output_lipson2020_fig5a_extended.txt
fi
